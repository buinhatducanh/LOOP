...
// ── Config builder ────────────────────────────────────────────────────────
function buildNeons(rank: RankKey, level: number, memberId: number): Neon[] {
  ...
  if (rank === 'iron') {
    ...
    // Diamond: 5 layers — phục hồi visual richness, vẫn GPU-composited
    mainCount = 5;
...
// ── LEDRunner — CSS-driven animation (GPU composited, no JS per-frame) ───
export const LEDRunner = memo(function LEDRunner({
  ...
  // ResizeObserver — avoid setState when dims unchanged
  useEffect(() => {
  ...
  // Inject CSS @keyframes once dims are known — CSS animation = GPU composited
  useEffect(() => {
    if (!dims) return;
    const [w, h] = dims;
    ...
    let css = '';
    neons.forEach((_, i) => {
      // Single keyframe per neon: 0 → -P (one full revolution)
  ...
  return (
    ...
    >
      ...
      >
        <defs>
          {/* Static diamond gradient — was motion.stop (costly JS per-frame) */}
          {member.rank === 'diamond' && (
          ...
          {/* Simplified glow filters: 1 gaussian blur instead of 3 */}
          {neons.map((n, i) =>
            n.hasGlow ? (
              <filter
                key={`f-${member.id}-${i}`}
        ...
        {neons.map((n, i) => {
          ...
          const startFrac   = isMain
            ? myMainIdx / mainCount + layerOffset / P
            : 0.15 + i * 0.25;

          // Negative delay = start mid-cycle at the correct position
