...
// ── Mission log item ──────────────────────────────────────────────────────
function MissionItem({ title, date, status, color, lpReward }: {
  title: string; date: string; status: string; color: string; lpReward?: number;
}) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-end gap-1 flex-shrink-0">
        ...
        {lpReward !== undefined && (
          <span style={{ color: color, fontSize: 9, fontFamily: "'JetBrains Mono', monospace", fontWeight: 700 }}>
            +{lpReward >= 1000 ? `${(lpReward / 1000).toFixed(0)}K` : lpReward} LP
          </span>
        )}
      </div>
    </div>
...
// ── Diamond Crown Badge (Diamond rank exclusive) ──────────────────────────
  ...
  return (
    <div style={{ position: 'relative', marginBottom: 20 }}>
      ...
      <div style={{ position: 'relative', borderRadius: 10, padding: 1.5, overflow: 'hidden' }}>
        {/* Rotating conic gradient — pure CSS, no JS per-frame */}
        <div
          style={{
            position: 'absolute',
            inset: '-150%',
            background:
              'conic-gradient(from 0deg, #818CF8, #7DD3FC, #F0ABFC, #FFFFFF, #7DD3FC, #818CF8, #818CF8)',
        ...
        >
          ...
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
            ...
            <svg width="26" height="22" viewBox="0 0 26 22" fill="none" style={{ flexShrink: 0 }}>
              <defs>
                <linearGradient id="hudCrownGrad" x1="0%" y1="0%" x2="100%" y2="0%">
              ...
              {/* Crown body */}
              <path
                d="M2 20 L5 8 L10.5 14.5 L13 2 L15.5 14.5 L21 8 L24 20 Z"
                fill="none"
                stroke="url(#hudCrownGrad)"
                strokeWidth="1.6"
                strokeLinejoin="round"
                strokeLinecap="round"
                filter="url(#hudCrownGlow)"
              />
              {/* Base bar */}
              <line x1="2" y1="20" x2="24" y2="20" stroke="url(#hudCrownGrad)" strokeWidth="1.8" strokeLinecap="round" />
              {/* Crown jewels */}
            ...
            <svg width="26" height="22" viewBox="0 0 26 22" fill="none" style={{ flexShrink: 0, transform: 'scaleX(-1)' }}>
              ...
              <path
                d="M2 20 L5 8 L10.5 14.5 L13 2 L15.5 14.5 L21 8 L24 20 Z"
                fill="none"
                stroke="url(#hudCrownGrad2)"
                strokeWidth="1.6"
                strokeLinejoin="round"
                strokeLinecap="round"
              />
              <line x1="2" y1="20" x2="24" y2="20" stroke="url(#hudCrownGrad2)" strokeWidth="1.8" strokeLinecap="round" />
        ...
        <>
          ...
          >
            ...
            <div className="p-6">
              ...
              {/* Diamond Crown Badge — exclusive for Diamond rank */}
              {member.rank === 'diamond' && <DiamondCrownBadge />}
              ...
              {/* LP Stats row — between XP panel and radar */}
              <div
                className="grid grid-cols-3 gap-2 mb-5"
              >
                {[
                  { label: 'BALANCE', value: member.lpBalance, accent: cfg.color },
                  { label: 'EARNED', value: member.lpEarned, accent: '#22C55E' },
                  { label: 'SPENT', value: member.lpSpent, accent: '#F59E0B' },
                ].map(({ label, value, accent }) => (
                  <div
                    key={label}
                    className="text-center px-2 py-2 rounded-lg"
                    style={{ background: '#111827', border: `1px solid ${accent}20` }}
                  >
                    <div
                      style={{
                        color: accent,
                    ...
                    <div style={{ color: '#475569', fontSize: 8, fontFamily: "'JetBrains Mono', monospace", letterSpacing: '0.1em', marginTop: 2 }}>
                      {label} LP
                    </div>
                  </div>
                ))}
