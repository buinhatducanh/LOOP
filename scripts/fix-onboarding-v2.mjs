...
// Fix 1: hexRgba function — add CSS var check
const hexRgbaOld = `function hexRgba(hex: string, alpha: number): string {
  const h = hex.replace("#", "");
...
const hexRgbaNew = `function hexRgba(hex: string, alpha: number): string {
...
if (content.includes(hexRgbaOld)) {
  content = content.replace(hexRgbaOld, hexRgbaNew);
  console.log('hexRgba fixed!');
} else {
  console.log('hexRgbaOld not found — trying find+replace by index');
  const idx = content.indexOf('function hexRgba');
  if (idx >= 0) {
    // Find the full function
    let bc = 0, start = -1, end = -1;
    for (let i = idx; i < content.length; i++) {
      if (content[i] === '{') { if (bc === 0) start = i; bc++; }
      if (content[i] === '}') { bc--; if (bc === 0) { end = i + 1; break; } }
    }
    if (start >= 0 && end >= 0) {
      const oldFn = content.slice(start, end);
      const newFn2 = hexRgbaNew.replace(/^function hexRgba[^{]*\{/, '').replace(/\}$/, '');
      content = content.slice(0, start) + '{' + hexRgbaNew.slice(hexRgbaNew.indexOf('{') + 1) + content.slice(end);
      console.log('hexRgba replaced by slice method');
    }
  }
}

// Fix 2: Replace StarField with SunnyBurst
...
if (content.includes('function StarField() {')) {
  ...
  console.log('StarField replaced!');
} else {
  console.log('StarField pattern not found, trying by index');
  const sfIdx = content.indexOf('function StarField() {');
  if (sfIdx >= 0) {
    // Find the closing }
    let bc = 0, end = -1;
    for (let i = sfIdx; i < content.length; i++) {
      if (content[i] === '{') bc++;
      if (content[i] === '}') { bc--; if (bc === 0) { end = i + 1; break; } }
    }
    if (end >= 0) {
      content = content.slice(0, sfIdx) + sunnyBurst + content.slice(end);
      console.log('StarField replaced by index!');
    }
  }
}
