...
> Ngày: 2026-03-24
> **Trạng thái**: ✅ **ALL PM MODULES COMPLETE** — Đang cập nhật: BA Ecosystem (LP Engine, Rank, Referral, EDU, Auto Pricing) — 2026-03-24
...
| LP Redemption cho khách hàng | ❌ | Không có chỗ đổi LP lấy dịch vụ |
...
| Tính năng | Trạng thái | Ghi chú |
|-----------|-----------|---------|
| Infrastructure Tier (Basic/Pro/Enterprise) | ❌ | Không có model Infra tier |
...
### F7. EDU SYSTEM — GIÁO DỤC (HOÀN TOÀN MỚI)
...
/admin/edu/courses/[id]       — Chi tiết + lessons
/admin/edu/enrollments        — Danh sách học viên + attendance
/admin/edu/feedback           — Xem feedback từ học viên
/admin/edu/attendance         — Điểm danh
```

**API routes cần tạo**:
```
/api/admin/edu/instructors     — CRUD instructor
/api/admin/edu/courses         — CRUD course + lessons
/api/admin/edu/enrollments     — Enrollment + attendance
/api/admin/edu/feedback        — Feedback CRUD
```
...
PHASE 3: AUTO PRICING
  ├── M10: Infrastructure Tier model (Basic/Pro/Enterprise) + pricing
  ├── M11: Price formula engine: Base + Feature + Infrastructure
  ├── M12: Auto-apply price khi Quote approved → Order
  └── M13: Public pricing calculator UI

PHASE 4: EDU SYSTEM
  ├── M14: Schema: Course, Instructor, Student, Enrollment
  ├── M15: Schema: Lesson, Attendance, Feedback, StudentProgress
  ├── M16: API routes + Admin pages (instructors, courses, enrollments)
  └── M17: EDU LP flow: student pays → LP to student + instructor

PHASE 5: RANK UI EFFECTS
  ├── M18: Rank card với 7 effects (Glow, Glitch, Aurora, Tail, Spark, Particles, Heartbeat)
  └── M19: Rank sync — rank cập nhật khi TeamMember LP thay đổi
```
