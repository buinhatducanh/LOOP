...
const PERMISSION_MATRIX: Record<string, Record<string, string[]>> = {
  ...
  content_manager: {
    services: ACTIONS, projects: ACTIONS, team: ACTIONS, testimonials: ACTIONS,
    messages: ["read", "update"], packages: ["read"], orders: ["read"],
  },
  sales_manager: {
    orders: ACTIONS, messages: ACTIONS, packages: ["read", "update"],
    services: ["read"], projects: ["read"], team: ["read"], testimonials: ["read"],
  },
  content_editor: {
    services: ["read", "create", "update"], projects: ["read", "create", "update"],
    team: ["read"], testimonials: ["read", "create", "update"],
  },
  sales_rep: {
    orders: ["read", "create", "update"], messages: ["read", "update"],
    packages: ["read"], services: ["read"], projects: ["read"],
  },
  viewer: Object.fromEntries(
    RESOURCES.filter((r) => r !== "settings" && r !== "audit_logs" && r !== "users").map((r) => [r, ["read"]])
  ),
};
