                    ...
                    <div key={f.id} className="flex items-center gap-3 rounded-lg border border-slate-800 bg-slate-900/80 px-4 py-2.5">
                      ...
                      <div className="flex gap-1.5">
                        {packages.map((pkg) => {
                          ...
                          return (
                            <span key={pkg.slug} className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${
                              val === true ? "bg-green-500/20 text-green-400" :
                              val === false ? "bg-slate-700 text-slate-500" :
...
function ComparisonFeatureModal({
  ...
  const [values, setValues] = useState<Record<string, { type: "boolean" | "text"; value: boolean | string }>>(
    () => {
      const result: Record<string, { type: "boolean" | "text"; value: boolean | string }> = {};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const [key, v] of Object.entries(values)) {
        finalValues[key] = v.type === "text" ? (v.value as string) : (v.value as boolean);
      }
              ...
              return (
                <div key={pkg.slug} className="flex items-center gap-3">
                  ...
                  <select
                    value={v.type}
                    onChange={(e) => {
                      const type = e.target.value as "boolean" | "text";
                      setValues((prev) => ({
                        ...prev,
                        [pkg.slug]: { type, value: type === "boolean" ? false : "" },
                      }));
                    }}
