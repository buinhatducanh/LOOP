 ...
 * Migrated from FE/src/app/store/loopStore.ts
 */
...
const INIT_ADMIN_NOTIFS: AdminNotification[] = [
  ...
  { id: "an7", type: "media_booking",   title: "📸 Booking mới — CafeHouse chụp ẩm thực", body: "Trần Minh Hùng (CafeHouse VN) đặt lịch chụp ảnh sản phẩm + ẩm thực. Gói Professional.", time: "6 giờ trước", timestamp: Date.now() - 6*3600000, read: false, color: DS.amber, relatedOrderId: "MDA-8821", priority: "normal", department: "media", assignedTo: "Haru Tanaka", category: "media", archived: false },
  { id: "an8", type: "media_delivery",  title: "🎬 Giao file — FashionX lookbook xong",   body: "Haru Tanaka đã giao toàn bộ ảnh lookbook Summer 2026 cho Phạm Thị Lan (FashionX). 120 ảnh.", time: "8 giờ trước", timestamp: Date.now() - 8*3600000, read: true, color: DS.cyan, relatedOrderId: "MDA-8824", priority: "normal", department: "media", assignedTo: "Haru Tanaka", category: "media", archived: false },
];
...
export const useLoopStore = create<LoopStore>((set, get) => ({
  ...
  updateOrderStatus: (orderId, status, pm) => set((s) => {
    const newNotif: AdminNotification = {
      ...
      timestamp: Date.now(), priority: "normal", department: "sales", assignedTo: "LOOP Team", category: "order", archived: false,
    };
    return {
  ...
  markAdminNotifRead: (id) =>
    set((s) => ({ adminNotifications: s.adminNotifications.map((n) => (n.id === id ? { ...n, read: true } : n)) })),

  markAllAdminNotifsRead: () =>
    set((s) => ({ adminNotifications: s.adminNotifications.map((n) => ({ ...n, read: true })) })),

  deleteAdminNotif: (id) =>
    set((s) => ({ adminNotifications: s.adminNotifications.filter((n) => n.id !== id) })),

  archiveAdminNotif: (id) =>
    set((s) => ({ adminNotifications: s.adminNotifications.map((n) => (n.id === id ? { ...n, archived: true } : n)) })),

  bulkDeleteAdminNotifs: (ids) =>
    set((s) => ({ adminNotifications: s.adminNotifications.filter((n) => !ids.includes(n.id)) })),

  bulkReadAdminNotifs: (ids) =>
    set((s) => ({ adminNotifications: s.adminNotifications.map((n) => (ids.includes(n.id) ? { ...n, read: true } : n)) })),

  bulkArchiveAdminNotifs: (ids) =>
    set((s) => ({ adminNotifications: s.adminNotifications.map((n) => (ids.includes(n.id) ? { ...n, archived: true } : n)) })),

  addAdminNotification: (notif) =>
    set((s) => ({ adminNotifications: [notif, ...s.adminNotifications] })),

  // ── Portfolio ────────────────────────────────────────────────────────────
  updateProject: (id, data) =>
    set((s) => ({ portfolio: s.portfolio.map((p) => (p.id === id ? { ...p, ...data } : p)) })),
  ...
  updateEffect: (id, data) =>
    set((s) => ({ effects: s.effects.map((e) => (e.id === id ? { ...e, ...data } : e)) })),
  ...
  toggleEffectEnabled: (id) =>
    set((s) => ({ effects: s.effects.map((e) => (e.id === id ? { ...e, enabled: !e.enabled } : e)) })),

  setGlobalEffectsEnabled: (enabled) => set({ globalEffectsEnabled: enabled }),

  updateMemberOverride: (override) =>
    set((s) => {
      const idx = s.memberEffectOverrides.findIndex(
        (o) => o.memberId === override.memberId && o.effectId === override.effectId
      );
      if (idx >= 0) {
  ...
  // ── Client actions ───────────────────────────────────────────────────────
  sendClientMessage: (orderId, content) =>
    set((s) => {
      const msg: OrderMessage = {
        id: `msg_${Date.now()}`, senderId: "client", senderName: "Khách hàng",
        content, type: "text",
        timestamp: new Date().toLocaleString("vi-VN"), read: false,
      };
  ...
  markClientNotifRead: (id) =>
    set((s) => ({ clientNotifications: s.clientNotifications.map((n) => (n.id === id ? { ...n, read: true } : n)) })),

  markAllClientNotifsRead: () =>
    set((s) => ({ clientNotifications: s.clientNotifications.map((n) => ({ ...n, read: true })) })),

  // ── Computed ─────────────────────────────────────────────────────────────
  getOrdersByClient: (clientId) => get().orders.filter((o) => o.clientId === clientId),

  getUnreadAdminCount: () =>
    get().adminNotifications.filter((n) => !n.read && !n.archived).length,
  ...
  getUserEquippedEffectObjects: () => {
    ...
    return userEquippedEffects
      .map((id) => effects.find((e) => e.id === id))
      .filter(Boolean) as RankEffect[];
  },
}));
