...
/**
 * Unified Login Page — /vi/dang-nhap
 *
 * Single login form for ALL account types (staff + customer).
 * Uses POST /api/admin/auth/login which correctly classifies accountType
 * based on teamMemberId (staff) vs no teamMemberId (customer).
 *
 * After login: shows success state on the same page.
 * Staff users click "Quản trị" in the header dropdown to go to admin.
 * Customer users click their name/avatar in the header to go to /khach-hang.
 *
 * Flow:
 *   User fills form → /api/admin/auth/login
 *   → 200: role === "client"  → stay on page, show customer dashboard link
 *   → 200: role !== "client" → stay on page, show "Vào Quản trị" button
 *   → 4xx: show error
 */
...
// ── Login result type ──────────────────────────────────────────────────────────
type LoginResult =
  | { ok: true; role: string; name: string; accountType: string }
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (ok) {
        ...
        setLoginResult({ ok: true, role, name: user?.name ?? email, accountType: state.accountType ?? "customer" });
        toast.success("Đăng nhập thành công!", {
          description: `Chào mừng ${user?.name ?? "bạn"}`,
        });
        // DO NOT redirect — stay on this page; header will update automatically
      } else {
  ...
  return (
    <div style={{
      ...
      display: "flex", position: "relative", fontFamily: DS.body,
    }}>
      {/* ── Ambient background ── */}
      ...
      <div style={{
        ...
        {/* SUCCESS STATE — shown after successful login */}
        {loginResult?.ok ? (
          <motion.div
            key="success"
            ...
            style={{
              ...
              border: `1px solid ${hexRgba(DS.green ?? "#22C55E", 0.3)}`,
          ...
          >
            <div style={{
              ...
              <CheckCircle2 size={28} color={DS.green ?? "#22C55E"} />
            </div>

            <h2 style={{
              fontFamily: DS.heading, fontSize: "1.25rem", fontWeight: 900,
            ...
            <p style={{ color: DS.text3, fontSize: "0.875rem", marginBottom: "2rem" }}>
              Xin chào <strong style={{ color: DS.text }}>{loginResult.name}</strong>
            </p>

            {/* CTA based on role */}
            {loginResult.role === "client" ? (
              <button
                onClick={() => router.push(`/${locale}/khach-hang`)}
                style={{
                  width: "100%",
            ...
            <form onSubmit={handleSubmit}>
              ...
              {error && (
                <div style={{
                  ...
                  color: DS.red ?? "#EF4444", fontSize: "0.8125rem", marginBottom: "1rem",
                }}>
                  {error}
                </div>
              )}
