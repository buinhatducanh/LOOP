...
export function mapLocalizedTeamMember<T extends LocalizedRecord>(
  ...
  return {
    ...
    lpEarned: record["lpEarned"] ?? record["lp_earned"] ?? 0,
    lpSpent: record["lpSpent"] ?? record["lp_spent"] ?? 0,
    team: record["team"] ?? "Alpha",
    roleCode: record["roleCode"] ?? record["role_code"] ?? "BOW",
    specialty: getLocalizedField(record, "specialty", locale),
    missions: record["missions"] ?? 0,
    challenge: getLocalizedField(record, "challenge", locale),
    solution: getLocalizedField(record, "solution", locale),
    result: getLocalizedField(record, "result", locale),
    // ── Career / skills ──────────────────────────────────────────
