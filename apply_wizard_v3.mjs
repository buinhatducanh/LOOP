// apply_wizard_v3.mjs — All BookingWizardClient.tsx changes in one pass
import fs from 'fs';
...
if (timingIdx >= 0) {
 ...
 console.log('0. Timing condition at line', timingIdx + 1);
}

// 1. Replace domain input section + purchase timing + domain price table
// Find the mb-6 just before TÊN MIỀN label
...
// Find the closing </div> of step 1 wrapper (the one after domain price table)
// Look for `</div>` + `)}` + ` {/* ── STEP 2:` pattern
let step1End = -1;
for (let k = labelIdx + 1; k < lines.length; k++) {
 if (lines[k].includes('{/* ── STEP 2:') || lines[k].includes('{/* STEP 2:')) {
 step1End = k - 1; // the </div> just before step 2
 // Go back to find the actual </div>
 for (let m = k - 1; m >= 0; m--) {
 if (lines[m].trim() === '</div>') { step1End = m; break; }
 }
 break;
 }
}
console.log('Domain section: mb-6 at line', mb6Idx + 1, ', step1 end at line', step1End + 1);
...
// Replace from mb6Idx to step1End (inclusive)
lines = lines.slice(0, mb6Idx).concat(newDomainSection).concat(lines.slice(step1End + 1));
...
if (schedIdx >= 0) {
 ...
 if (schedStart >= 0) {
 // Count depth properly: process line by line
 let depth = 0;
 let schedEnd = -1;
 for (let k = schedStart; k < lines.length; k++) {
 const l = lines[k];
 // Simple: each line either opens with <div or closes with </div>
 const opens = (l.match(/<div /g) || []).length + (l.match(/<div>/g) || []).length;
 const closes = (l.match(/<\/div>/g) || []).length;
 if (depth === 0) depth += opens; // first line has the opening tag
